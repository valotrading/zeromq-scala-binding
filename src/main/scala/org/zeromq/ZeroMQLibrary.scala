...
object ZeroMQ {
  ...
  val ZMQ_NOBLOCK = 1
  val ZMQ_DONTWAIT = 1 // replaces noblock in 3.2
  val ZMQ_SNDMORE = 2

  /** Socket options */
  val ZMQ_HWM = 1
  val ZMQ_SWAP = 3 // removed in 3.2
  val ZMQ_AFFINITY = 4
  val ZMQ_IDENTITY = 5
  val ZMQ_SUBSCRIBE = 6
...
trait ZeroMQLibrary extends Library {
  ...
  // TODO def zmq_ctx_set_monitor(void *context, zmq_monitor_fn *monitor)   //http://api.zeromq.org/3-2:zmq-ctx-set-monitor
}
